...
export const getScaledCard = (baseCard: CardData, ngPlusLevel: number): CardData => {
    ...
    if (scaledCard.buyCost && scaledCard.buyCost > 0) {
        isModified = true;
        const baseCost = scaledCard.buyCost;
...
export function getAttackPowerBreakdown(card: CardData, playerDetails: PlayerDetails, source: CardContext): string | null {
    ...
    if (card.effect.type === 'conditional_weapon') {
        ...
        const hasAnotherKnife = playerDetails.hand.some(c => c && c.id !== card.id && (c.id.includes('_knife') || c.effect?.subtype === 'sword')) || playerDetails.equippedItems.some(c => c.id !== card.id && (c.id.includes('_knife') || c.effect?.subtype === 'sword'));
    ...
    if (card.effect.type === 'conditional_weapon') {
        ...
        if (card.effect.condition === 'is_knife') { 
            const hasAnotherKnife =
                playerDetails.hand.some(c => c && c.id !== card.id && (c.id.includes('_knife') || c.effect?.subtype === 'sword')) ||
                playerDetails.equippedItems.some(c => c.id !== card.id && (c.id.includes('_knife') || c.effect?.subtype === 'sword'));
            if (hasAnotherKnife) {
                attackPower += card.effect.bonus_attack || 0;
            }
        }
...
export function getFormattedEffectText(card: CardData, source: CardContext, playerDetails?: PlayerDetails): string | null {
    ...
    switch (effect.type) {
        ...
        case 'upgrade':
            ...
            if (effect.subtype === 'bladed_multiplier') return `Multiplies bladed weapon damage by ${effect.multiplier || 2}.`;
...
export function isEventConsideredHostile(eventCard: CardData | null): boolean {
    ...
    // Rule: A thief is always hostile for trade, regardless of health.
    if (eventCard.immediateEffect?.type === 'random_gold_steal' && (eventCard.immediateEffect.maxAmount || 0) > 0) {
        return true;
    }

    // Rule: Any other threat with 5 or less health is NOT hostile for trading.
    ...
    // Mountain Sickness is not hostile for trade
