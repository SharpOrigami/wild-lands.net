    ...
    switch (objective.id) {
        case 'objective_take_em_alive':
            if (isPeacefulVictory) {
                ...
                failureLog = `Optional Objective Failed.\nThe final blow of ${lastAttackPower} damage was too powerful to take the boss alive.`;
            }
            break;
        ...
        case 'objective_the_purist':
            if (player.currentIllnesses.length === 0) {
                success = true;
                ...
                const illnessNames = player.currentIllnesses.map(ill => ill.name).join(', ');
        ...
        case 'objective_the_hoarder':
            if (player.gold >= 250) {
                ...
                rewardLog = `Congratulations!\nYour prosperity is proven with ${player.gold} Gold!\n\nReward: You'll start your next journey with an additional +50 Gold.`;
            } else {
                failureLog = `Optional Objective Failed.\nYou only had ${player.gold} Gold upon victory.`;
            }
            break;
        case 'objective_a_beasts_end':
            if (boss.subType === 'animal') {
                success = true;
                localStorage.setItem('objectiveReward_a_beasts_end_WWS', 'true');
                rewardLog = `Congratulations!\nThe final boss was a beast!\n\nReward: You'll start your next journey with an AI-remixed legendary themed fur coat.`;
            } else {
                failureLog = `Optional Objective Failed.\nThe final boss was not a beast.`;
            }
            break;
        ...
        case 'objective_the_last_stand':
            if (player.health <= 5) {
                ...
                rewardLog = `Congratulations!\nA victory snatched from the jaws of death with ${player.health} HP remaining!\n\nReward: Your lineage gains a permanent +2 Max Health.`;
            } else {
                failureLog = `Optional Objective Failed.\nYou had ${player.health} HP, which was not low enough.`;
            }
            break;
        case 'objective_well_prepared':
            const provisionsCollected = parseInt(localStorage.getItem('provisionsCollected_WWS') || '0', 10);
        ...
        case 'objective_master_trapper':
            if (player.runStats.animals_killed_by_trap >= 3) {
                success = true;
                localStorage.setItem('objectiveReward_master_trapper_WWS', 'true');
                rewardLog = `Congratulations!\nYou trapped ${player.runStats.animals_killed_by_trap} animals!\n\nReward: You'll start your next run with an AI-remixed legendary 'themed trap'.`;
            } else {
                failureLog = `Optional Objective Failed.\nYou only trapped ${player.runStats.animals_killed_by_trap} animals.`;
            }
            break;
    }
...
export const applyImmediateEventAndCheckEndTurn = (
    ...
    if (modifiablePlayer.activeTrap && eventForProcessing && eventForProcessing.type === 'Event' && (eventForProcessing.subType === 'animal' || eventForProcessing.subType === 'human')) {
        ...
        const trapInteractionResult = handleTrapInteractionWithEvent(modifiablePlayer, eventForProcessing, modifiablePlayer.activeTrap, isBossEvent, triggerBanner, triggerAnimation, _log);
        ...
        if (trapInteractionResult.goldFromBossDefeat && trapInteractionResult.goldFromBossDefeat > 0) {
            ...
            if (isBossEvent) {
              ...
              lastAttackPowerRef.current = trapInteractionResult.damageDealt;
            }
        }
    ...
    if (!eventForProcessing) {
        return { updatedPlayer: modifiablePlayer, turnEndedByEvent, gameShouldEnd, eventRemoved: true, winReason, damageInfo: damageInfoForAnimation, modifiedEventAfterTrap: null };
    }
    ...
    } else if (currentEventInstance.subType === 'environmental' || eventId.startsWith('threat_thief_')) {
        const bannerType: ActiveGameBannerState['bannerType'] = eventEffect?.turn_end ? 'turn_ending_event' : 'event_alert';
        triggerBanner(currentEventInstance.name, bannerType, eventEffect?.turn_end);
    }
    ...
    if (currentEventInstance.type === 'Event' &&
        (
          (currentEventInstance.subType === 'human' && !eventId.startsWith('threat_thief_') && !eventId.startsWith('threat_vagabond_')) ||
          (currentEventInstance.subType === 'animal' && (currentEventInstance.health || 0) > 8 && !eventId.startsWith('threat_skunk_') && !eventId.startsWith('threat_rattlesnake_'))
        ) &&
        eventEffect?.type === 'damage' && (eventEffect.amount || 0) > 0) {

        _log(getRandomLogVariation('enemyAttackImmediate', { enemyName: currentEventInstance.name }, theme, modifiablePlayer, currentEventInstance, isBossEvent), 'event');
    ...
    return { updatedPlayer: modifiablePlayer, turnEndedByEvent, gameShouldEnd, eventRemoved: eventRemovedInitially || !eventForProcessing, winReason, damageInfo: damageInfoForAnimation, modifiedEventAfterTrap: eventRemovedInitially ? null : eventForProcessing };
};